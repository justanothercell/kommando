...
void resolve_expr(Program* program, Module* module, FuncDef* func, Expression* expr, VarList* vars) {
    switch (expr->type) {
        ...
        case EXPR_FUNC_CALL: {
            ...
            if (fd->return_type == NULL) fd->return_type = gen_typevalue("::std::unit", &fd->name->span);
        ...
        case EXPR_STRUCT_LITERAL: {
            ...
            resolve_typevalue(program, module, slit->type);
            ...
            map_foreach(slit->fields, lambda(void, (str key, StructFieldLit* field) {
                ...
                assert_types_equal(program, module, field->value->resolved->type, f->type, field->name->span);
            }));
