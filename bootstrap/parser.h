...
#define spanned_error(title, span, message, ...) do { \
    if (span.right.line > span.left.line) { \
        while (span.right.line > span.left.line) { \
            span.right.index -= 1; \
            if (span.right.source[span.right.index] == '\n') { \
                span.right.line -= 1; \
            } \
        } \
        span.right.column = span.left.column + (span.right.index - span.left.index); \
    } \
